function [] = plotTurkValTasks()
    ...
    nPoints     = [8, 8, 11, 11];   % Number of points in the observed psychometric and confidence functions [task 1, task 2]
    ...
    for ib = 1:numel(batches) % batches 1-4
        ...
        for iT = 1:numel(tasks) % crdm, cdd
            ...
            fileList    = dir(fullfile(outPath,batch,[tasks{iT},'_bestParams_subjects_*.mat'])); 
            % Load data
            load(fullfile(fileList.folder,fileList.name),'trials', 'bestParamEst', 'bestNLL','subList');
            ...
            for iO = 1:nSubjects
                ...
                fprintf('Plotting data for subject %d of %d, task %s... \n', iO, nSubjects, tasks{iT});
                ...
                for iR = 1:nRel
                    ...
                    params   = bestParamEst{iO}{1}([1, 2+(iR-1), 2+nRel, 3+nRel, 4+nRel:end]);
                ...
                %     axis([-5 5 0 1])
                    ...
                    if iR == nRel
                        ...
                        title(sprintf('Subject %s',subject),'Interpreter', 'none');
                    end
                ...
                %     axis([-5 5 0 1])
                    ...
                    metauncertainty = bestParamEst{iO}{1}(3+nRel);
                    % subject = subList(iO).name(1:30);
                    subject_metauncertainty(iO,:) = {subject,metauncertainty};
                
                    text(0,.85,sprintf('Decision criterion = %.2g \nMeta-uncertainty = %.2g \nConfidence criterion = %.2g \nLapse rate = %.2g\n ',bestParamEst{iO}{1}(2+nRel),bestParamEst{iO}{1}(3+nRel),bestParamEst{iO}{1}(4+nRel),bestParamEst{iO}{1}(1)));
                    
                    interLine       = .1;
                    linePosition    = .6;
                    for itextR = 1:nRel
                        text(0,linePosition,sprintf('Sensitivity%d = %.2g',itextR,bestParamEst{iO}{1}(2+(itextR-1))));
                        linePosition = linePosition-interLine;
                    end
                    text(0,linePosition,sprintf('NLL = %.4g',bestNLL{1}{1}));
                ...
                saveas(gcf,fullfile(figFolder,[tasks{iT} '_' subject '.eps']),'epsc');
                close
            end
            subj_meta_fn = fullfile(outPath,batch,[tasks{iT},'_metauncertainty_by_subject.csv']);
